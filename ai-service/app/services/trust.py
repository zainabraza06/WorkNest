...
DUMMY IMPLEMENTATION — the same transparent weighted heuristic the backend uses as a
fallback (backend/src/services/trust.service.js), kept deliberately in sync so the two
are interchangeable.

Swap-in plan: train a logistic regression / gradient boosting model on (simulated)
historical outcomes with exactly these features and return its calibrated probability
× 100. The response contract stays the same.
"""

MODEL_NAME = "dummy-weighted-v1"
...
def score_worker(f) -> dict:
    completion = f.completed_jobs / f.total_jobs if f.total_jobs else 0.7
    ...
    score = round(100 * _clamp01(0.5 * (1 - confidence) + raw * confidence + verified_bonus))
